import { ADMIN_WORKER_URL, MEDIA_WORKER_URL, MEMBERSHIP_WORKER_URL, EVENTS_WORKER_URL } from './config';
...
export async function getMembers(token) {
  const res = await fetch(`${MEMBERSHIP_WORKER_URL}/admin/members`, { headers: authHeaders(token) });
  return handleResponse(res);
}

export async function approveMember(token, id) {
  const res = await fetch(`${MEMBERSHIP_WORKER_URL}/admin/approve`, {
    method: 'POST',
    headers: authHeaders(token),
...
export async function rejectMember(token, id) {
  const res = await fetch(`${MEMBERSHIP_WORKER_URL}/admin/reject`, {
    method: 'POST',
    headers: authHeaders(token),
    body: JSON.stringify({ id }),
  });
...
export async function renewMember(token, id) {
  const res = await fetch(`${MEMBERSHIP_WORKER_URL}/admin/renew`, {
    method: 'POST',
    headers: authHeaders(token),
    body: JSON.stringify({ id }),
  });
...
export async function getRegistrations(token, eventSlug) {
  const url = eventSlug
